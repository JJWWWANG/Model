...
def train(args, model, dataset, optimizer, epochs, device):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        while data is not None:
            ...
            result = model(x)  # batchx22 12 + 8 + 2 实际上是21
            #print("result",result.size())   #result.size() 64 21
            if model.task.lower() == 'ex':
                loss = model.get_ex_loss(result, labels['EX'])
            elif model.task.lower() == 'au':
                loss = model.get_au_loss(result, labels['AU'])
...
def main(args):
    ...
    elif opt['model_name'] == 'tformer2':
        model = SpatialTemporalFormer2(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
    elif opt['model_name'] == 'urlformer':
        model = SpatialTemporalFormer3(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
    elif opt['model_name'] == 'setformer':
        model = SpatialTemporalFormer4(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
    elif opt['model_name'] == 'mtformer':
        model = SpatialTemporalFormer5(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
    elif opt['model_name'] == 'msformer':
        model = SpatialTemporalFormer6(modality=args['modality'], task=task, video_pretrained=False, num_patches=args['n_frames'])
