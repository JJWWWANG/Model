...
class ResFormer(nn.Module):  # S-Former after stage3
    ...
    def __init__(self, block, Mblock, SEblock, layers, zero_init_residual=False,
        ...
        self.layer10 = self._make_layer(block, 64, layers[0])
        self.layer20 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.inplanes = 64
        self.layer11 = self._make_layer2(Mblock, 64, 2, stride=1)
        self.layer21 = self._make_layer(SEblock, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.Fusion = CCMFusion(128, 128)
    ...
    def forward(self, x):
        ...
        x = self.layer10(x)  # torch.Size([1, 64, 28, 28])
        x = self.layer20(x)  # torch.Size([1, 128, 14, 14])
        x2 = self.layer11(x2)  # torch.Size([1, 64, 28, 28])
        x2 = self.layer21(x2)  # torch.Size([1, 128, 14, 14])
        FusionX = self.Fusion(x, x2)

        x = self.layer3(FusionX)  # torch.Size([1, 256, 7, 7])
...
class SpatialTemporalFormer6(nn.Module):
    def __init__(self, modality='A;V;M', video_pretrained=True, task='AU', num_patches=16):
        super(SpatialTemporalFormer6, self).__init__()

        self.video_model = VideoModel(num_patches=num_patches)
        if video_pretrained:
            load_pretrain_sformer(self.video_model, r'./experiments/msformer_AU_MT/pretrain/latest.pth')
