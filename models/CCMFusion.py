...
class FusionAttention(nn.Module):
    ...
    def pad(self, x, ps):
        _, _, H, W = x.size()
        if W % ps != 0:
            x = F.pad(x, (0, 2), mode='constant') #(0, ps - W % ps)原来是 reflect
        if H % ps != 0:
...
if __name__ == '__main__':
    ...
    block = CCMFusion(128,128)
    input1 = torch.rand(1, 128, 14, 14)
    input2 = torch.rand(1, 128, 14, 14)
...
# x2 = x
#
# x = self.layer1_0(x)  # torch.Size([1, 64, 28, 28])
# x = self.layer2_0(x)  # torch.Size([1, 128, 14, 14])
#
# x2 = self.layer1_1(x2)  # torch.Size([1, 64, 28, 28])
# x2 = self.layer2_1(x2)  # torch.Size([1, 128, 14, 14])


# self.layer1_0 = self._make_layer(block, 64, layers[0])
# self.layer1_1 = self._make_layer2(Mblock, 64, 2, stride=1)
# self.layer2_0 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
# self.layer2_1 = self._make_layer(SEblock, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])